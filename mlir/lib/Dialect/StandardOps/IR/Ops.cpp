...
static LogicalResult simplifyBitcastOfBitcast(BitcastOp op,
                                              PatternRewriter &rewriter) {
  auto *sourceOp = op.getOperand().getDefiningOp();
  if (auto sourceBitcast = dyn_cast_or_null<BitcastOp>(sourceOp)) {
    rewriter.updateRootInPlace(
        op, [&] { op->setOperands({sourceBitcast.getOperand()}); });
    return success();
  }
  return failure();
}
...
void BitcastOp::getCanonicalizationPatterns(RewritePatternSet &results,
                                            MLIRContext *context) {
  results.add(&simplifyBitcastOfBitcast);
}
